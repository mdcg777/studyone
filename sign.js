...
function get_ms_token(randomlength = 120) {
    //根据传入长度产生随机字符串
    let i = 0;
    let random_str = '';
    let base_str = 'ABCDEFGHIGKLMNOPQRSTUVWXYZabcdefghigklmnopqrstuvwxyz0123456789=';
    let le = base_str.length - 1;

    while (i < randomlength) {
        random_str += base_str[getRandomInt(0, le)];
        i++;
    }
...
module.exports = { sign_datail, sign_reply, get_ms_token, getRandomInt };